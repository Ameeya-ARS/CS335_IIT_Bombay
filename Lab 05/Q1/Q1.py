...
def w_closed_form(X, Y):
    ...
    temp1 = torch.matmul(torch.t(X),X)
    temp2 = torch.inverse(temp1)
    temp3 = torch.matmul(temp2,torch.t(X))
    temp4 = torch.matmul(temp3,Y)
...
def l1_loss(X, Y, w):
    ...
    new_w = w.reshape(w.shape[0],1)
    y_pred = torch.matmul(X,new_w)
    temp = torch.abs(Y - y_pred)
    loss = torch.mean(torch.abs(temp))
...
def l2_loss_derivative(X, Y, w):
    ...
    X_T = torch.t(X)
    new_w = w.reshape(w.shape[0],1)
    temp = torch.matmul(X,new_w) - Y
    derivative = (2/n)*(torch.matmul(X_T,temp))
